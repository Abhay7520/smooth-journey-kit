            ...
            <div className="divide-y divide-white/[0.06]">
              {notifications.map(n => (
                <div key={n.id} className={`flex items-start gap-3 px-5 py-3 transition-colors hover:bg-white/[0.04] ${n.unread ? "bg-orange-500/[0.03]" : ""}`}>
                  <div className={`mt-1 h-2 w-2 shrink-0 rounded-full ${n.type === "delivery" ? "bg-blue-400" : n.type === "delivered" ? "bg-emerald-400" : n.type === "update" ? "bg-orange-400" : "bg-violet-400"
                    }`} />
            ...
            {trackingTimeline.map((step, i) => (
              <div key={step.label} className="flex flex-col items-center gap-1.5">
                <div className="flex items-center">
                  <div className={`flex h-6 w-6 items-center justify-center rounded-full ${step.done ? "bg-orange-500 shadow-lg shadow-orange-500/30" : "bg-white/[0.08]"
                    }`}>
                    {step.done && <span className="text-[10px] text-white">✓</span>}
                  </div>
                  {i < trackingTimeline.length - 1 && (
                    <div className={`h-0.5 w-8 sm:w-12 lg:w-16 ${step.done && trackingTimeline[i + 1]?.done ? "bg-orange-500" :
          ...
          <div className="divide-y divide-white/[0.06]">
            {recentOrders.map((o) => (
              <div key={o.id} className="flex items-center gap-4 p-5 hover:bg-white/[0.04] transition-colors">
                <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${o.status === "Delivered" ? "bg-emerald-500/10" :
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-medium ${o.status === "Delivered" ? "bg-emerald-500/10 text-emerald-400" :
                ...
                <div className="hidden sm:flex flex-col items-end gap-1">
                  ...
                  <div className="h-1.5 w-24 rounded-full bg-white/[0.08]">
                    <div className={`h-full rounded-full ${o.status === "Delivered" ? "bg-emerald-500" :
                      o.status === "In Transit" ? "bg-blue-500" : "bg-amber-500"
                      }`} style={{ width: `${o.progress}%` }} />
                  </div>
                </div>
                <span className={`hidden md:block text-xs font-medium ${o.risk === "None" ? "text-emerald-400" : o.risk === "Low" ? "text-blue-400" : "text-amber-400"
                  }`}>
